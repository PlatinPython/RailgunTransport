...
public class RailgunUpdateSelectedTargetPKT {
    ...
    public static RailgunUpdateSelectedTargetPKT decode(FriendlyByteBuf buffer) {
        return new RailgunUpdateSelectedTargetPKT(buffer.readBlockPos(), buffer.readOptional(FriendlyByteBuf::readBlockPos));
    }
    ...
    public static class Handler {
        public static void handle(RailgunUpdateSelectedTargetPKT message, Supplier<NetworkEvent.Context> context) {
            context.get().enqueueWork(() -> {
                ...
                if (blockEntity instanceof RailgunBlockEntity railgunBlockEntity) {
                    railgunBlockEntity.setSelectedTarget(message.selectedTarget);
                    railgunBlockEntity.tryCapsuleSend();
                }
            });
